// lib/logo.ts
// Utilitas normalisasi dan pembatasan logo usaha (TUGAS 2).
...
export async function normalizeLogo(file: File): Promise<string> {
  ...
  // 2. Terima hanya image/png, image/jpeg, image/webp. Tolak image/svg+xml
  const validTypes = ["image/png", "image/jpeg", "image/webp"]
  if (!validTypes.includes(file.type)) {
    throw new Error("Tipe berkas tidak didukung (gunakan PNG, JPEG, atau WebP)")
  }

  // 3. Muat ke ImageBitmap atau HTMLImageElement
  ...
  // 4. Encode ke PNG (512px). Jika > 200KB -> WebP 0.85 -> 384px -> 256px
  let resultDataUrl = await renderCanvasDataUrl(imgSource, origW, origH, LOGO_MAX_DIMENSION, "image/png")
  if (getDataUrlByteSize(resultDataUrl) <= LOGO_MAX_BYTES) {
    return resultDataUrl
  }

  // Coba WebP 0.85 di 512px
  resultDataUrl = await renderCanvasDataUrl(imgSource, origW, origH, LOGO_MAX_DIMENSION, "image/webp", 0.85)
  if (getDataUrlByteSize(resultDataUrl) <= LOGO_MAX_BYTES) {
    return resultDataUrl
  }

  // Coba WebP 0.85 di 384px
  resultDataUrl = await renderCanvasDataUrl(imgSource, origW, origH, 384, "image/webp", 0.85)
  if (getDataUrlByteSize(resultDataUrl) <= LOGO_MAX_BYTES) {
    return resultDataUrl
  }

  // Coba WebP 0.85 di 256px
  resultDataUrl = await renderCanvasDataUrl(imgSource, origW, origH, 256, "image/webp", 0.85)
  return resultDataUrl
}
...
export async function normalizeLogoDataUrl(dataUrl: string): Promise<string | null> {
  ...
  try {
    ...
    result = await renderCanvasDataUrl(img, origW, origH, LOGO_MAX_DIMENSION, "image/webp", 0.85)
    if (getDataUrlByteSize(result) <= LOGO_MAX_BYTES) return result

    result = await renderCanvasDataUrl(img, origW, origH, 384, "image/webp", 0.85)
    if (getDataUrlByteSize(result) <= LOGO_MAX_BYTES) return result

    result = await renderCanvasDataUrl(img, origW, origH, 256, "image/webp", 0.85)
    return result
  } catch {
    return null
  }
